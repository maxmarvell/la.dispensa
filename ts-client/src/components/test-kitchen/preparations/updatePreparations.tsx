...
import { MutateInstructionProps } from "../../../@types/test-kitchen";
import { InstructionType, TimeAndTemperatureType } from "../../../@types/preparations";
import { CreateInstructionProps } from "../../../@types/test-kitchen";
...
const UpdateField = ({ instruction, setNodes }: MutateInstructionProps) => {
  ...
  const [instructionInput, setInput] = useState<Partial<InstructionType>>(timeAndTemperature ? data : rest);
  useEffect(() => (
  ...
  const handleSetHours = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setInput(({ timeAndTemperature, ...rest }) => {
      let newField = timeAndTemperature as TimeAndTemperatureType;
      if (hours !== "") {
        newField.hours = parseInt(hours);
      } else {
  ...
  const handleSetMinutes = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setInput(({ timeAndTemperature, ...rest }) => {
      let newField = timeAndTemperature as TimeAndTemperatureType;
      if (minutes !== "") {
        newField.minutes = parseInt(minutes);
      } else {
  ...
  const handleSetTemperature = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setInput(({ timeAndTemperature, ...rest }) => {
      let newField = timeAndTemperature as TimeAndTemperatureType;
      if (temperature !== "") {
        newField.temperature = parseInt(temperature);
      } else {
...
export const UpdatePreparations = ({ instructions, setNodes }: CreateInstructionProps) => {
  return (
    <>
