...
const ComponentCard = ({ recipe, setActive }) => {
  return (
    <button
...
const ActiveComponentCard = ({ data }) => {
  ...
  const { mutateAsync: removeConnection } = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(['componentSearch, components'])
    }
  })
...
const DetailedRecipe = ({ recipe, setActive }) => {
...
export default function Components() {
  ...
  const { user: { id: authorId } } = useContext(AuthContext);
  const { recipeId } = useParams();
  ...
  const { isPending, isError, data: components, error } = useQuery({
