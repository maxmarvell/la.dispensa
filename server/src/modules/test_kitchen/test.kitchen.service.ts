...
export async function createIteration(input: CreateIterationInput) {
  ...
  let parentInstructions : any = parentId ? (
    await prisma.instructionIteration.findMany({
      where: {
        iterationId: parentId,
        NOT: {
          timeAndTemperature: null
        }
      },
      include: {
        timeAndTemperature: true
  ...
  const instructions = parentInstructions.map(({ timeAndTemperature: { iterationId, recipeId , step, ...other}, ...rest }) => {
    return {
      ...rest,
      timeAndTemperature:{
        create: other
